        ...
        <div class="flex flex-wrap -m-2">
          ...
          <div class="p-2 w-full">
            <div class="relative">
              ...
              <textarea id="message" name="message" v-model="message" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out"></textarea>
            </div>
          </div>
          <div class="p-2 w-full">
            <a :href="getMailtoLink">
              <button class="flex mx-auto text-white bg-black border-0 py-2 px-8 focus:outline-none hover:bg-gray-600 rounded text-lg">
                {{ buttonPrimaryText }}
              </button>
            </a>
          </div>
...
export default {
  ...
  computed: {
    getMailtoLink() {
      return `mailto:528512@student.fontys.nl?subject=Contact DagBlik&body=Name: ${this.name}%0D%0AEmail: ${this.email}%0D%0AMessage: ${this.message}`;
    },
  },
};
</script>
